...
- **Stats that reflect how the season actually went** — KDA isn't independently rolled per game; it's derived from the season's win/loss margin (a 15–3 split reads very differently from a 9–9 one) and from how the playoff run went, with a "carry" adjustment so a standout player doesn't get dragged down by a bad team.
